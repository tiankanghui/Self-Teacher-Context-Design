...
# The official MCLM parquet stores one row per AIME problem and one column per
# language.  Keep the source column order stable so all checkpoints see exactly
# the same expanded example order before data-parallel sharding.
MT_AIME2024_LANGUAGES = [
    "af", "sq", "ar", "bn", "bg", "ca", "zh-cn", "zh-tw", "hr", "cs", "da",
    "nl", "en", "et", "fi", "fr", "de", "el", "gu", "he", "hi", "hu", "id",
    "it", "ja", "kn", "ko", "lv", "lt", "mk", "ml", "mr", "ne", "no", "fa",
    "pl", "pt", "pa", "ro", "ru", "sk", "sl", "so", "es", "sw", "sv", "tl",
    "ta", "te", "th", "tr", "uk", "ur", "vi", "cy",
]


def expand_mt_aime2024(dataset, requested_languages):
    """Convert MCLM's 30-row wide table into problem-language examples."""
    available = set(dataset.column_names) - {"answer"}
    if requested_languages == ["all"]:
        languages = MT_AIME2024_LANGUAGES
    else:
    ...
    if unknown:
        raise ValueError(
            f"Unknown MT-AIME2024 language columns: {unknown}. "
            f"Available columns: {sorted(available)}"
        )

    records = []
    for problem_index, example in enumerate(dataset):
        for language in languages:
            problem = example[language]
            if problem is None or not str(problem).strip():
                raise ValueError(
                    f"Empty MT-AIME2024 problem at row={problem_index}, language={language}"
                )
            records.append(
                {
                    "problem": str(problem),
                    ...
                    "language": language,
                    "source_problem_index": problem_index,
                }
            )

    expected = len(dataset) * len(languages)
    if len(records) != expected:
        raise RuntimeError(f"MT-AIME2024 expansion failed: got {len(records)}, expected {expected}")
    return Dataset.from_list(records), languages
...
def load_vllm_model(
    ...
    Args:
        ...
        distributed_executor_backend: vLLM executor backend. ``auto`` uses the
            single-process executor for one-GPU evaluation and multiprocessing
            only when tensor parallelism spans multiple GPUs.
    ...
    if distributed_executor_backend == "auto":
        # vLLM 0.11 defaults world-size-one inference to ``uni``.  Avoid
        # forcing ``mp`` here: launching several one-GPU shards concurrently
        # otherwise creates unnecessary TCP rendezvous servers and can fail
        # nondeterministically with EADDRINUSE.
        distributed_executor_backend = "uni" if tensor_parallel_size == 1 else "mp"
    llm_config["distributed_executor_backend"] = distributed_executor_backend
    print(f"vLLM distributed executor backend: {distributed_executor_backend}")
...
def evaluate_math500(
    ...
    selected_languages = []
    if eval_data_path and ds_name == "mt-aime2024":
        data_file = Path(eval_data_path) / "mt-aime2024" / "mt-aime2024.parquet"
        if not data_file.is_file():
            raise FileNotFoundError(f"MT-AIME2024 parquet not found: {data_file}")
        wide_dataset = load_dataset(
            "parquet", data_files={"train": str(data_file)}, split="train"
        )
        dataset, selected_languages = expand_mt_aime2024(
            wide_dataset, languages or ["all"]
        )
        print(
            f"Loaded local MT-AIME2024 from {data_file}: "
            f"{len(wide_dataset)} problems x {len(selected_languages)} languages "
            f"= {len(dataset)} examples"
        )
    elif eval_data_path and ds_name in ("aime24", "aime25", "hmmt25"):
        # Load from local parquet files
        # Dataset name mapping: aime24 → aime2024, aime25 → aime2025, hmmt25 → hmmt2025
        ds_dir_map = {"aime24": "aime2024", "aime25": "aime2025", "hmmt25": "hmmt2025"}
        ds_dir = f"{eval_data_path}/{ds_dir_map[ds_name]}"
        data_files = f"{ds_dir}/train*.parquet"
        dataset = load_dataset("parquet", data_files={"train": data_files}, split="train")
        print(f"Loaded local {ds_name} dataset from {ds_dir} with {len(dataset)} problems")
    elif ds_name == "math500":
        dataset = load_dataset("HuggingFaceH4/MATH-500", split="test")
        print(f"Loaded HuggingFaceH4/MATH-500 dataset with {len(dataset)} problems")
    elif ds_name == "amo-bench":
        ...
        raise ValueError(
            f"Unknown dataset: {dataset_name}. Choose 'math500', 'amo-bench', 'aime24', "
            f"'aime25', 'hmmt25', 'mt-aime2024', 'minerva', or 'amc23'"
        )
    ...
    # Apply sharding for data-parallel evaluation
    if num_shards > 1:
        shard_size = (len(dataset) + num_shards - 1) // num_shards
        start_idx = shard_id * shard_size
        end_idx = min(start_idx + shard_size, len(dataset))
        dataset = dataset.select(range(start_idx, end_idx))
        print(f"Shard {shard_id}/{num_shards}: evaluating problems {start_idx}-{end_idx-1} ({len(dataset)} problems)")
    ...
    for example in dataset:
        ...
        if dataset_name.lower() == "amo-bench":
            ...
            gt_answer = str(example["answer"])
            question_id = example.get("problem_idx", None)
        elif dataset_name.lower() == "mt-aime2024":
            problem = example["problem"]
            gt_answer = str(example["answer"])
            language = example["language"]
            source_problem_index = example["source_problem_index"]
            question_id = f"{source_problem_index}:{language}"
        else:
            # MATH500 format
            problem = example["problem"]
        ...
        all_languages.append(example.get("language", None))
        all_source_problem_indices.append(example.get("source_problem_index", None))
    ...
    # Process results
    print("\nProcessing results...")
    for idx, (output, problem, gt_answer, question_id, language, source_problem_index) in enumerate(
        zip(
            outputs,
            all_problems,
            all_gt_answers,
        ...
        result = {
            ...
            "language": language,
            "source_problem_index": source_problem_index,
            "val_n": val_n,
            "generations": [
    ...
    # MT-AIME2024-specific breakdown.  Store counts as well as percentages so
    # multiple data-parallel shards can be merged without averaging percentages.
    language_metrics = {}
    if ds_name == "mt-aime2024":
        for language in selected_languages:
            language_results = [r for r in results if r["language"] == language]
            if not language_results:
                continue
            language_correct = sum(r["num_correct"] for r in language_results)
            language_total = sum(r["val_n"] for r in language_results)
            language_metrics[language] = {
                "num_problems": len(language_results),
                "correct": language_correct,
                "total_solutions": language_total,
                "average_at_n_pct": language_correct / language_total * 100,
            }

    english_average_at_n_pct = None
    non_english_macro_average_at_n_pct = None
    language_gap_pct = None
    if "en" in language_metrics:
        english_average_at_n_pct = language_metrics["en"]["average_at_n_pct"]
    non_english_scores = [
    ...
    if non_english_scores:
        non_english_macro_average_at_n_pct = sum(non_english_scores) / len(non_english_scores)
    if english_average_at_n_pct is not None and non_english_macro_average_at_n_pct is not None:
        language_gap_pct = english_average_at_n_pct - non_english_macro_average_at_n_pct
    ...
    print(f"  Format rate: {format_rate:.2f}%")
    if language_metrics:
        print(f"\nMT-AIME2024 language breakdown:")
        if english_average_at_n_pct is not None:
            print(f"  English Avg@{val_n}: {english_average_at_n_pct:.2f}%")
        if non_english_macro_average_at_n_pct is not None:
            print(
                f"  Non-English Macro Avg@{val_n}: "
                f"{non_english_macro_average_at_n_pct:.2f}%"
            )
        if language_gap_pct is not None:
            print(f"  Language Gap (English - Non-English): {language_gap_pct:.2f} points")
    print("=" * 70)
    ...
    if output_file:
        ...
        summary = {
            ...
            "english_average_at_n_pct": english_average_at_n_pct,
            "non_english_macro_average_at_n_pct": non_english_macro_average_at_n_pct,
            "language_gap_pct": language_gap_pct,
            "results": results,
        }
...
def main():
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Path to local eval dataset directory (e.g., 'opsd_data/eval_data/'). "
        "If set, loads from local parquet instead of HuggingFace.",
    )
    parser.add_argument(
        "--languages",
        nargs="+",
        default=["all"],
        help=(
            "MT-AIME2024 language columns to evaluate, e.g. '--languages en zh-cn fr'. "
            "Use 'all' for all 55 languages (default)."
        ),
    )
    parser.add_argument("--shard_id", type=int, default=0,
                        help="Shard index for data-parallel eval (0-based, use with --num_shards)")
    ...
    parser.add_argument(
        ...
        default="auto",
        help=(
            "vLLM executor backend (default: auto; uni for one GPU and mp for "
